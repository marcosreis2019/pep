...
@Component({
  selector: 'app-solicitacao-exames-form',
  templateUrl: './solicitacao-exames-form.component.html',
  styleUrls: ['./solicitacao-exames-form.component.scss']
})
export class SolicitacaoExamesFormComponent implements OnInit {
  ...
  save() {
    this.loading = true
    this.documento.documentos_tipos_id = this.documentType.id
    this.payload = { ...this.documento }
    this.payload.observacao = this.payload.observacao.replace(/\n/g, '<br/>')
    delete this.payload.data_emissao
    this.payload.nome_alterado =
      this.documento.nome_alterado && this.documento.beneficiario_nome !== this.beneficiarioNomeAux

    this.documentoService.post(this.payload).subscribe(
      res => {
